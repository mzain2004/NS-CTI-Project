...
from services.report_service import generate_report_exports, generate_report
...
async def generate_report(request: ReportGenerateRequest):
    try:
        ...
        metadata, exports = generate_report_exports(placeholder_analysis, request.analyst, request.formats)
        return {'metadata': metadata.model_dump(), 'exports': [item.model_dump() for item in exports]}
    except Exception as exc:
        return JSONResponse(status_code=500, content={'error': f'report_generate_failed: {exc}'})
...
async def generate_report_endpoint(request: ReportGenerateRequest):
    try:
        ...
        return JSONResponse(status_code=500, content={"error": f"Failed to generate report: {exc}"})
...
@router.get('/api/report/{report_id}/json')
...
@router.get('/api/report/{report_id}/iocs')
...
async def delete_report(report_id: str):
    try:
        ...
        return JSONResponse(status_code=500, content={"error": f"Failed to delete report: {exc}"})
